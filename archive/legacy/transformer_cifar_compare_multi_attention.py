...
def prepare_token_views(config, data, include_test_views=True):
    ...
    return {
        ...
        "view1_te": add_pos(view1_te) if view1_te is not None else None,
        "view2_te": add_pos(view2_te) if view2_te is not None else None,
...
def run_closed_form_transformer(config):
    ...
    views = prepare_token_views(config, data, include_test_views=False)
    ytr = data["ytr"]
    yte = data["yte"]
    ytr_onehot = one_hot(ytr, int(np.max(ytr) + 1), dtype=np.float32)
    yhat_tr = np.zeros_like(ytr_onehot)
    yhat_te = np.zeros((len(yte), ytr_onehot.shape[1]), dtype=np.float32)

    base_tr = views["base_tr"].astype(np.float32)
    base_te = views["base_te"].astype(np.float32)
    view1_tr = views["view1_tr"].astype(np.float32)
    view2_tr = views["view2_tr"].astype(np.float32)
