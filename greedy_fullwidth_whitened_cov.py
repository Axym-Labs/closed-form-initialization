...
def run_experiment(suite_name, depth, final_dim, lambda_reg):
    ...
    return {
        ...
        "layers": layers,
        "note": (
            "Full-width one-parameter whitened layer with objective "
...
def main():
    parser = argparse.ArgumentParser(description="Greedy full-width one-parameter whitened layer on MNIST.")
    parser.add_argument(
        "--suite",
    ...
    print(
        f"Greedy full-width whitened-cov layer  |  suite={result['suite']}  |  depth={result['depth']}  |  final_d={result['final_dim']}"
    )
